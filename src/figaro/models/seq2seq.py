...
from figaro.constants import PAD_TOKEN, EOS_TOKEN, BAR_KEY, POSITION_KEY
...
class Seq2SeqModule(pl.LightningModule):
    def __init__(
        ...
        assert self.description_flavor in [
            "latent",
            "description",
            "none",
            "both",
    ...
    def decode(self, x, labels=None, bar_ids=None, position_ids=None, encoder_hidden_states=None, return_hidden=False):
        ...
        if encoder_hidden_states is not None:
            ...
            out = self.transformer.decoder(
                inputs_embeds=x_emb, encoder_hidden_states=encoder_hidden_states, output_hidden_states=True
            )
    ...
    def forward(
        self, x, z=None, labels=None, position_ids=None, bar_ids=None, description_bar_ids=None, return_hidden=False
    ):
    ...
    def get_loss(self, batch, return_logits=False):
        ...
        logits = self(
            x, z=z, labels=labels, bar_ids=bar_ids, position_ids=position_ids, description_bar_ids=desc_bar_ids
        )
    ...
    def sample(
        ...
        for i in range(curr_len - 1, max_length):
            ...
            logits = self.decode(
                x_, bar_ids=bar_ids_, position_ids=position_ids_, encoder_hidden_states=encoder_hidden_states
            )
            ...
            next_positions = [
                int(token.split("_")[-1]) if f"{POSITION_KEY}_" in token else None for token in next_positions
            ]
            next_positions = [

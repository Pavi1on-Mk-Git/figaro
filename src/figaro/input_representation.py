from figaro.chord_recognition import MIDIChord
import numpy as np
import pretty_midi
...
class Event(object):
    ...
    def __repr__(self):
        return "Event(name={}, time={}, value={}, text={})".format(self.name, self.time, self.value, self.text)
...
class InputRepresentation:
    ...
    def _read_items(self):
        ...
        for time, tempo in zip(times, tempi):
            self.tempo_items.append(Item(name="Tempo", start=time, end=None, velocity=None, pitch=int(tempo)))
        ...
        for tick in wanted_ticks:
            if tick in existing_ticks:
                ...
                output.append(
                    Item(
                        name="Tempo", start=self.pm.time_to_tick(tick), end=None, velocity=None, pitch=output[-1].pitch
                    )
                )
    ...
    def get_remi_events(self):
        ...
        for i in range(len(self.groups)):
            ...
            if current_chord is not None:
                events.append(
                    Event(name=POSITION_KEY, time=0, value="{}".format(0), text="{}/{}".format(1, positions_per_bar))
                )
                events.append(
                    Event(
                        name=CHORD_KEY,
            ...
            for item in self.groups[i][1:-1]:
                ...
                if item.name == "Note":
                    ...
                    events.append(Event(name=INSTRUMENT_KEY, time=item.start, value=name, text="{}".format(name)))
                    # pitch
                    events.append(
                        Event(
                            name=PITCH_KEY,
                    ...
                    if current_chord is None or item.pitch != current_chord.pitch:
                        events.append(pos_event)
                        events.append(
                            Event(name=CHORD_KEY, time=item.start, value=item.pitch, text="{}".format(item.pitch))
                        )
                        current_chord = item
    ...
    def get_description(self, omit_time_sig=False, omit_instruments=False, omit_chords=False, omit_meta=False):
        events = []
        n_downbeat = 0
        current_chord = None

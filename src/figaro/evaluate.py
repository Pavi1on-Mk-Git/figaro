...
import figaro.input_representation as ir
    ...
    for sample_id, group in enumerate(file_groups):
        ...
        for orig_file, sample_file in group:
            ...
            for g1, g2, cg1, cg2 in zip(orig.groups, sample.groups, chord_groups1, chord_groups2):
                ...
                meta1, meta2 = meta_stats(g1, ticks_per_beat=orig.pm.resolution), meta_stats(
                    g2, ticks_per_beat=sample.pm.resolution
                )
                row["pitch_oa"] = overlapping_area(
                    meta1["pitch_mean"], meta1["pitch_std"], meta2["pitch_mean"], meta2["pitch_std"]
                )
                row["velocity_oa"] = overlapping_area(
                    meta1["velocity_mean"], meta1["velocity_std"], meta2["velocity_mean"], meta2["velocity_std"]
                )
                row["duration_oa"] = overlapping_area(
                    meta1["duration_mean"], meta1["duration_std"], meta2["duration_mean"], meta2["duration_std"]
                )
        ...
        print(
            "[info] Group {}: inst_f1={:.2f} | chord_f1={:.2f} | pitch_oa={:.2f} | vel_oa={:.2f} | dur_oa={:.2f} | chroma_sim={:.2f} | groove_sim={:.2f}".format(
                sample_id + 1,
                micro_avg["inst_f1"],
                micro_avg["chord_f1"],
                micro_avg["pitch_oa"],
                micro_avg["velocity_oa"],
                micro_avg["duration_oa"],
                micro_avg["chroma_sim"],
                micro_avg["groove_sim"],
            )
        )

...
def parse_args():
    ...
    parser.add_argument(
        "--vae_checkpoint", type=str, default=None, help="Path to the VQ-VAE model checkpoint (optional)"
    )
    parser.add_argument(
        "--lmd_dir", type=str, default="./lmd_full", help="Path to the root directory of the LakhMIDI dataset"
    )
...
def main():
    ...
    if args.make_medleys:
        dl = medley_iterator(
            dl, n_pieces=args.n_medley_pieces, n_bars=args.n_medley_bars, description_flavor=model.description_flavor
        )

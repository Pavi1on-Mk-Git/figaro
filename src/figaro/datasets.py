...
LATENT_CACHE_PATH = os.getenv(
    "LATENT_CACHE_PATH", os.path.join(os.getenv("SCRATCH", os.getenv("TMPDIR", "./temp")), "latent")
)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.description_flavor in ["latent", "both"]:
            assert (
                self.vae_module is not None
            ), "Description flavor 'latent' requires 'vae_module' to be present, but found 'None'"
...
class MidiDataset(IterableDataset):
    ...
    def load_file(self, file):
        ...
        try:
            ...
            sample = {"events": events, "description": description}
